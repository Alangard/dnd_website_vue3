    ...
    <div class="comment-item_container" 
        ...
        <div class="comment_header">
            <comment-info-header
                ...
                @moreFuncSwipeElementOpen="showModal('bottom_sheet')"
                @startEditComment="startEditComment()"
                @showDeleteAlert="showModal('delete_alert')">
            </comment-info-header>
        </div>
...
import { ref, defineProps, defineEmits, defineAsyncComponent, watch, computed} from 'vue'
...
const checkModalActionStatus = computed(() => { return store.getters.getModalActionStatus});

function showModal(modal_type){
    store.state.modals_info.id = props.item.id;
    store.state.modals_info.state = true;
    store.state.modals_info.modal_type = modal_type;
}

watch(checkModalActionStatus, () => {
        if(store.state.modals_info.id == props.item.id && store.state.modals_info.modal_type =='delete_alert'){
            deleteComment();
            store.state.modals_info.id = 0;
            store.state.modals_info.action_pressed = false;
